...
if(must.install){
  ...
  biocLite("Link-NY/RTCGAToolbox", "vjcitn/MultiAssayExperiment", "waldronlab/BiocInterfaces")
}
```
...
dses <- lapply(choices, function(choice) try(BiocInterfaces::TCGAextract(prad, choice),
                                             silent=TRUE))
names(dses) <- choices
dses
```
...
```{r, cache=TRUE}
eset.rnaseq <- TCGAextract(prad, "rnaseq2genenorm")
eset.rnaseq$racevar = mergeVecs(as.character(eset.rnaseq$patient.clinical_cqcf.race),  
                                as.character(eset.rnaseq$patient.race))
